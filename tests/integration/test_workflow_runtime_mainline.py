...
def _require_env(name: str) -> str:
    ...
    if not value:
        pytest.skip(f"{name} is required for workflow runtime integration test")
    return value
...
async def _clear_runtime_tables() -> None:
    ...
    async with db.session_factory() as session:  # noqa: SLF001 - integration setup phase19去掉_session前面的_
...
async def _run_workflow_worker_until_idle() -> None:
    ...
    worker = _build_worker("workflow", queue, store, "workflow-worker-it")
...
async def _create_workflow_definition(
    *,
    ...
    definition = WorkflowDefinition(
        ...
        checksum="it-workflow-checksum",
        created_by="workflow-it",
        updated_by="workflow-it",
    )
...
async def test_workflow_submit_relay_worker_e2e():
    os.environ["REDIS_URL"] = "redis://127.0.0.1:6379"
    ...
    assert persisted.queue_name == "workflow_tasks"
    ...
    assert task_body["result"]["workflow_key"] == workflow_key
    assert task_body["result"]["workflow_version"] == workflow_version
    assert task_body["result"]["workflow_status"] == "succeeded"
    assert task_body["result"]["active_node_ids"] == []
    assert task_body["result"]["created_step_count"] == 1
    assert task_body["result"]["executed_step_count"] == 1
    assert "draft_quote" in task_body["result"]["step_outputs"]
    ...
    assert execution.input_json["customer_name"] == "Acme"
    assert execution.output_json is not None
    assert execution.output_json["executed_step_count"] == 1
    ...
    assert step_items[0].status.value == "succeeded"
    assert step_items[0].output_json is not None
    assert step_items[0].retry_policy_json["max_attempts"] == 1

    event_items = await _get_event_items(execution.workflow_execution_id)
    assert len(event_items) == 6
    assert event_items[0].event_type.value == "execution.created"
    assert event_items[1].event_type.value == "execution.running"
    assert event_items[2].event_type.value == "step.created"
    assert event_items[3].event_type.value == "step.running"
    assert event_items[4].event_type.value == "step.succeeded"
    assert event_items[5].event_type.value == "execution.succeeded"

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        execution_resp = await client.get(
            f"/api/v1/workflow/executions/by-task/{task_id}"
        )
        assert execution_resp.status_code == 200, execution_resp.text

        execution_body = execution_resp.json()
        assert execution_body["task_id"] == task_id
        assert execution_body["workflow_key"] == workflow_key
        assert execution_body["workflow_version"] == workflow_version
        assert execution_body["status"] == "succeeded"
        assert execution_body["active_node_ids"] == []