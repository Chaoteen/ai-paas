...
async def _run_workflow_worker_until_idle() -> None:
    ...
    worker = _build_worker("workflow", queue, store, "workflow-product-retry-exhausted-worker-it")
...
async def _create_workflow_definition(
    *,
    ...
    definition = WorkflowDefinition(
        ...
        display_name="Product Planning Flow Retry Exhausted",
        ...
        checksum="workflow-product-retry-exhausted-it",
...
async def _submit_workflow_product_retry_exhausted(
    *,
    product_key: str,
) -> dict:
    transport = ASGITransport(app=app)
            ...
            json={
                ...
                "input_json": {
                    ...
                    "fail_n_times_by_node_id": {
                        "draft_product_plan": 2,
                    },
                },
                "context_json": {"operator": "pm_assistant"},
                "metadata_json": {"request_channel": "web"},
                "trigger_source": "workflow_product_api",
                "idempotency_key": f"product-retry-exhausted-{unique}",
                "correlation_id": f"product-retry-exhausted-{unique}",
            },
        )
...
async def test_workflow_product_retry_exhausted_mainline() -> None:
    ...
    submit_body = await _submit_workflow_product_retry_exhausted(product_key=product_key)
    ...
    assert second_attempt.status == WorkflowStepExecutionStatus.FAILED
    ...
    assert WorkflowExecutionEventType.STEP_FAILED in event_types
    assert WorkflowExecutionEventType.EXECUTION_FAILED in event_types