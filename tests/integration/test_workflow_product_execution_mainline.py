...
async def _create_workflow_definition(
    *,
    ...
    definition = WorkflowDefinition(
        ...
        display_name="Product Planning Flow",
        ...
        checksum="workflow-product-success-it",
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.post(
            ...
            json={
                ...
                "idempotency_key": f"product-exec-success-{unique}",
                "correlation_id": f"product-exec-success-{unique}",
            },
        )
...
async def test_workflow_product_execution_mainline_success() -> None:
    ...
    assert step_items[0].status == "succeeded"
