...
def _require_env(name: str) -> str:
    ...
    if not value:
        pytest.skip(f"{name} is required for workflow product execution integration test")
    return value
...
async def _run_workflow_worker_until_idle() -> None:
    ...
    worker = _build_worker("workflow", queue, store, "workflow-product-worker-it")
...
async def _create_workflow_definition(
    *,
    ...
    definition = WorkflowDefinition(
        ...
        display_name="Product Planning Flow Failure",
        ...
        checksum="workflow-product-failure-it",
        created_by="workflow-product-it",
        updated_by="workflow-product-it",
    )
...
async def _submit_workflow_product_failure(
    *,
    product_key: str,
) -> dict:
    transport = ASGITransport(app=app)
        ...
        response = await client.post(
            ...
            json={
                ...
                "input_json": {
                    ...
                    "force_fail_node_id": "draft_product_plan",
                },
                "context_json": {"operator": "pm_assistant"},
                "metadata_json": {"request_channel": "web"},
                "trigger_source": "workflow_product_api",
                "idempotency_key": f"product-exec-failure-{unique}",
                "correlation_id": f"product-exec-failure-{unique}",
            },
        )
...
async def _workflow_product_execution_env_guard():
...
async def test_workflow_product_execution_mainline_failure() -> None:
    ...
    submit_body = await _submit_workflow_product_failure(
        product_key=product_key,
    )
    ...
    assert step_items[0].node_id == "draft_product_plan"
    assert step_items[0].status == "failed"
